...
import {searchReportDetail} from "../routes/reportDetailsRoute";
...
class SearchingView extends React.Component {
    ...
    onSubmit = event => {
        ...
        searchReportDetail(report, this.props.currentUser.token)
            .then(res => res.json())
                        ...
                        <div className="col-6 shadow overflow-auto" style={{height: 550}}>
                            <ul className="list-group">
                                {this.state.arr.map(report => {
                                    return <li onClick={() =>
                                    {this.setState({redirectTo: `/my-report/${report.reportDetailsId}`})}}
                                        key={report.reportDetailsId} className="list-group-item">
